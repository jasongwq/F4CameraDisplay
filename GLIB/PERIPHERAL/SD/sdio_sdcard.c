...
/*---------------------------- SDIO CMD Configuration ------------------------*/  
  ...
  tmpreg |= (uint32_t)cmdindex | waitrsp << 6
           | 0 << 8 | 1 << 10;
...
SD_Error SD_ReadBlock(u8* buf, u32 addr, u16 blksize)
{
    ...
    else if (DeviceMode == SD_DMA_MODE)
    {
        ...
        while (((DMA2->LISR & SD_SDIO_DMA_FLAG_TCIF) == RESET) && (TransferEnd == 0) && (TransferError == SD_OK) && timeout)timeout--; //等待传输完成
...
SD_Error SD_ReadMultiBlocks(u8* buf, u32 addr, u16 blksize, u32 nblks)
{
    ...
    if (nblks > 1)                                      //多块读
    {
        ...
        else if (DeviceMode == SD_DMA_MODE)
        {
            ...
            timeout = SDIO_DATATIMEOUT;
            while (((DMA2->LISR & SD_SDIO_DMA_FLAG_TCIF) == RESET) && timeout)timeout--; //等待传输完成
            if (timeout == 0)return SD_DATA_TIMEOUT; //超时
...
void SD_DMA_Config(u32* mbuf, u32 bufsize, u8 dir)
{
  ...
  SDDMA_InitStructure.DMA_Memory0BaseAddr = (u32)mbuf;
  SDDMA_InitStructure.DMA_DIR = dir<<6;
  SDDMA_InitStructure.DMA_BufferSize = bufsize/4;
...
void show_sdcard_info(void)
{
    ...
    {
    case SDIO_STD_CAPACITY_SD_CARD_V1_1: Sys_Printf(Printf_USART,"Card Type:SDSC V1.1\r\n"); break;
    case SDIO_STD_CAPACITY_SD_CARD_V2_0: Sys_Printf(Printf_USART,"Card Type:SDSC V2.0\r\n"); break;
    case SDIO_HIGH_CAPACITY_SD_CARD:     Sys_Printf(Printf_USART,"Card Type:SDHC V2.0\r\n"); break;
    case SDIO_MULTIMEDIA_CARD:           Sys_Printf(Printf_USART,"Card Type:MMC Card \r\n"); break;
    }
    Sys_Printf(Printf_USART,"Card ManufacturerID:%d\r\n", SDCardInfo.SD_cid.ManufacturerID);   //制造商ID
    Sys_Printf(Printf_USART,"Card RCA:%d\r\n", SDCardInfo.RCA);                              	//卡相对地址
    Sys_Printf(Printf_USART,"Card Capacity:%d MB\r\n", (u32)(SDCardInfo.CardCapacity >> 20)); 	//显示容量
    Sys_Printf(Printf_USART,"Card BlockSize:%d\r\n\r\n", SDCardInfo.CardBlockSize);          	//显示块大小
}
